...
static std::vector<int> parseGpuIdList(const std::string& s)
{
    std::vector<int> ids;
...
std::optional<std::vector<int>> checkIfDeviceTypeIsGPU(po::variables_map& map)
{
    std::optional<std::vector<int>> gpuIDs = std::nullopt;
    if (map.count("gpu"))
    {
        auto val = map["gpu"].as<std::string>();
        gpuIDs = parseGpuIdList(val);
        map.erase("gpu");
        std::cout << "Using GPU(s) with ID(s)=";
        for (size_t i = 0; i < gpuIDs->size(); ++i)
        {
            std::cout << gpuIDs->at(i) << (i + 1 < gpuIDs->size() ? "," : "");
        }
        std::cout << " backend for NVIDIA optimization.\n";
    }
    return gpuIDs;
}
...
int main (int argc, char *argv[])
{
    ...
    if (gpuIDs.has_value())
    {
        ...
        {
            device = std::make_shared<CudaDevice>(gpuIDs->front());
        }
